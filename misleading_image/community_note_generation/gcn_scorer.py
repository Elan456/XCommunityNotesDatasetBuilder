...
from misleading_image.twc import TweetWithContext
import json 
import pandas as pd 
import itertools
import re 
...
def good_or_not(tweet_text, tweet_image, generated_cn: str, original_cn: str):
    """
    Gives a community note a score of 0 or 1 based on whether it matches the original community note.
    It does not need to match exactly, but it should be relatively close or better.
    """

    gemini = Gemini("misleading_image/google.key")
    prompt = [
        "Below are two community notes, one is the original from Twitter, another is generated by an LLM.",
        "Assume the original community note is entirely true and correct.",
        "Check if the generated community note is accurate compared to the original community note.",
        "If the generated community note contains more or different sources it may still be accurate or even better.",
        "Check for any fundamental factual differences.",
        "If the generated community note simply focuses on a different aspect of the original community note, it may still be accurate.",
        """Give the results in the following format {"accurate": 0, "reason": "The generated note is missing some key details"}""",
        "'accurate' must be 0 or 1.",
        "Be sure to use double quotes for the JSON keys and values.",
        "# Tweet Text:",
        tweet_text,
        "# Tweet Image:",
        tweet_image,
    ...
    try:
        ...
        print("Response text: ", response_text)
        print("Trying again...")
        return good_or_not(tweet_text, tweet_image, generated_cn, original_cn)

    assert 'accurate' in response_json, "Response JSON does not contain 'accurate' key"
    assert int(response_json['accurate']) in [0, 1], "Response JSON 'accurate' key is not 0 or 1"

    # Get boolean value of accurate
    is_accurate = None
    if response_json['accurate'] == 1:
        is_accurate = True
    elif response_json['accurate'] == 0:
        is_accurate = False
    else:
        raise ValueError("Response JSON 'accurate' key is not 0 or 1")
    
    return is_accurate, response_json['reason']

def compare(tweet_text, tweet_image, original_cn: str, generated_cn_a: str, generated_cn_b: str) -> tuple[str, str]:
    """
...
def rank(df: pd.DataFrame, original_column: str, generated_columns: list[str], count: int = 100):
    ...
    all_accuracies_counts = {col: 0 for col in generated_columns}
    row_count = 0

    for _, row in df.iterrows():

        twc = TweetWithContext(row['text'], row['image_urls'][0], row[original_column])
        
        # Load tweet image with PIL
        original_cn = row[original_column]

        # Calculate accuracies for each generated note using godd_or_not
        this_row_accuracies = {col: 0 for col in generated_columns}
        this_row_accuracies_reasons = {col: "" for col in generated_columns}
        for col in generated_columns:
            generated_cn = row[col]
            is_accurate, reason = good_or_not(twc.text, twc.image, generated_cn, original_cn)
            all_accuracies_counts[col] += 1 if is_accurate else 0
            this_row_accuracies[col] = 1 if is_accurate else 0
            this_row_accuracies_reasons[col] = reason

        row_count += 1
        print(f"Row {row_count}/{num_rows}")
        ...
        while matchups_for_this_row > 0:
            ...
            print("Column".ljust(20), "ELO".ljust(20), "%Accuracy".ljust(20), end="\n")  
            for col, elo in reversed(sorted_elo):
                print(f"{col}".ljust(20), end="")
                print(f"{elo:.2f}".ljust(20), end="")
                print(f"{(all_accuracies_counts[col] / row_count) * 100:.2f}%".ljust(20), end="\n")
            ...
            while True:
                try:
                    result = compare(twc.text, twc.image, original_cn, generated_cn_a, generated_cn_b)
                    break
            ...
            results.append({
                ...
                "A's Accuracy": this_row_accuracies[a],
                "B's Accuracy": this_row_accuracies[b],
                "A's Accuracy Reason": this_row_accuracies_reasons[a],
                "B's Accuracy Reason": this_row_accuracies_reasons[b],
            })
    ...
    for col in generated_columns:
        ranking_results.append({
            ...
            "Accuracy": (all_accuracies_counts[col] / row_count) * 100,
            "Accuracy_Count": all_accuracies_counts[col],
        })
